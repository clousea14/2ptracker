...
//$passwordname = "test";//$_POST['luspass'];
// get all products from products table
$result = $mysqli->query("SELECT *FROM user_dmn where email='$emailname' and Password='$passwordname'");
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        // temp user array
        $product = array();
        $product["user_lastName"] = $row["user_lastName"];
        $product["user_firstName"] = $row["user_firstName"];
